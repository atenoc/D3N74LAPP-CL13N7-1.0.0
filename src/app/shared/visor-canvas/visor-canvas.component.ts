...
@Component({
  selector: 'app-visor-canvas',
  templateUrl: './visor-canvas.component.html',
  styleUrls: ['./visor-canvas.component.css']
})
export class VisorCanvasComponent implements AfterViewInit {
  ...
  dibujarCapas() {
    fabric.Image.fromURL('assets/images/odontograma.jpg', (imagen) => {
      imagen.data = { isBackground: true };
      imagen.selectable = false;
      imagen.evented = false;
  ...
  guardarEnNube() {
    ...
    // Extraer la parte de Base64 del dataURL
    ...
    this.uploadsService.uploadImage(uploadData).subscribe(res => {
      ...
      Alerts.error(Mensajes.ERROR_500, Mensajes.IMAGEN_NO_SUBIDA, Mensajes.INTENTAR_MAS_TARDE);
    });
  }
